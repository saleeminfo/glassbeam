...
ansible $HOST -m copy -a "src=~/bin/duel_eph_lvm-800_config.sh dest=~/bin/ mode=0755"

wait
echo -e "\nConfiguring File System(s), please wait ..."
ansible $HOST -m shell -a "~/bin/duel_eph_lvm-800_config.sh $ENV"
...
ansible $HOST -m shell -a "rm -r ~/bin/duel_eph_lvm-800_config.sh"
