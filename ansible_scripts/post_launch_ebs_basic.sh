...
ansible $HOST -m copy -a "src=~/bin/ebs_simple.sh dest=~/bin/ mode=0755"

wait
echo -e "\nConfiguring File System(s), please wait ..."
ansible $HOST -m shell -a "~/bin/ebs_simple.sh $ENV"

echo -e "\nDeploying /etc/fstab ..."
ansible $HOST -m copy -a "src=~/deploy/base/wide/system/etc/ebs_simple dest=/etc/fstab mode=0644" -s
...
ansible $HOST -m shell -a "rm -r ~/bin/ebs_simple.sh"
