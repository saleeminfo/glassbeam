...
ansible $HOST -m copy -a "src=~/bin/ebs-lvm-250_eph_config.sh dest=~/bin/ mode=0755"

wait
echo -e "\nConfiguring File System(s), please wait ..."
ansible $HOST -m shell -a "~/bin/ebs-lvm-250_eph_config.sh $ENV"
...
ansible $HOST -m shell -a "rm -r ~/bin/ebs-lvm-250_eph_config.sh"
