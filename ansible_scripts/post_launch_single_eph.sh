...
wait
echo -e "\nDeploying Host Name change script ..."
...
ansible $HOST -m copy -a "src=~/bin/single_eph_lvm-800_config.sh dest=~/bin/ mode=0755"

wait
echo -e "\nConfiguring File System(s), please wait ..."
ansible $HOST -m shell -a "~/bin/single_eph_lvm-800_config.sh $ENV"

echo -e "\nDeploying /etc/fstab ..."
ansible $HOST -m copy -a "src=~/deploy/base/wide/system/etc/eph_lvm_fstab dest=/etc/fstab mode=0644" -s
...
ansible $HOST -m shell -a "rm -r ~/bin/single_eph_lvm-800_config.sh"
